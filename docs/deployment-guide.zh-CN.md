...
- 把 `yukiho.site` 这个域名指向你的服务器公网 IP
- 让浏览器知道访问这个域名时应该去找哪台服务器
- 为后续 `Certbot` 申请 HTTPS 证书做准备

重要提醒：

- 域名解析是在**域名控制台 / DNS 控制台**里完成
- 这一步不是在 `Xshell` 终端里完成
- 如果 DNS 没配对，后面的 `Certbot` 大概率会失败
...
请登录你购买域名的平台或当前托管 DNS 的平台，然后找到类似下面名字的页面：
...
1. 登录域名平台控制台
2. 找到你的域名 `yukiho.site`
3. 进入“域名解析”或“DNS 记录”页面
4. 添加两条新的解析记录
...
- `@`：表示裸域名，也就是 `yukiho.site`
- `www`：表示 `www.yukiho.site`
- `A` 记录：表示把域名指向一个 IPv4 地址
- 记录值：就是你的云服务器公网 IP
- TTL：如果控制台里有这个字段，第一次可以先保持默认值
...
- 用户访问 `www.yukiho.site` 时也能到同一台服务器
- 然后再由 `Nginx` 把 `www` 跳转到裸域名
...
- 返回的 IP 与你的服务器公网 IP 一致
- 两个域名都能解析到这台服务器

如果解析刚添加，可能需要等待几分钟到几小时不等才会完全生效。  
刚保存后暂时查不到，不一定代表你配错了。
...
1. 先完成域名解析
2. 再确认 `http://yukiho.site` 已经能访问到你的服务器
3. 最后再执行 `certbot` 申请 HTTPS

不要反过来做。  
如果 DNS 还没指向这台服务器，`certbot` 在验证域名时通常无法通过。
